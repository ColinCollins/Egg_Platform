    ...
    public class BaseUIViewEditor : UnityEditor.Editor
    {
        ...
        private void GenerateUIComponents()
        {
            ...
            EditorApplication.delayCall += () =>
            {
                ...
                EditorApplication.delayCall += () =>
                {
                    BindComponentsToInspector(targetView, components);
                };
            };
        ...
        private void BindComponentsToInspector(BaseUIView targetView, List<UIComponentInfo> components)
        {
            if (targetView == null)
            {
            ...
            {
                Debug.LogWarning($"[BaseUIViewEditor] 未找到以下字段（可能脚本尚未编译完成）: {string.Join(", ", notFoundFields)}");
            }
            
            Debug.Log($"[BaseUIViewEditor] 已自动绑定 {boundCount}/{components.Count} 个组件到 Inspector");
        }
