...
{
    public class LegacyAudioCollectionEditorWindow : EditorWindow
    {
        ...
        // [MenuItem("Tools/Melenitas Dev/Sounds Good/Legacy/Audio Collection", false, 101)]
        public static void ShowWindow ()
        {
            // Get existing open window or if none, make a new one:
            LegacyAudioCollectionEditorWindow window =
                (LegacyAudioCollectionEditorWindow)GetWindow(typeof(LegacyAudioCollectionEditorWindow));
            window.Show();
        ...
        {
            tryDeleteAll = false;
            soundDataCollection = AssetLocator.Instance.SoundDataCollection;
            musicDataCollection = AssetLocator.Instance.MusicDataCollection;
            ...
            if (changesDetected)
            {
                ...
                if (!nullAudioClip)
                {
                    if (GUILayout.Button("Update Clips", greenButtonStyle))
                    {
                        ...
                        EditorHelper.ChangeAudioClipImportSettings(currentDataCollection[index].Clips,
        ...
        private void UpdateTag (SoundData soundData, int index)
        {
            ...
            if (GUILayout.Button("Update Tag", greenButtonStyle))
            {
                ...
                using (EnumGenerator enumGenerator = new EnumGenerator())
                {
                    ...
                    string enumLocationPath = AssetDatabase.GetAssetPath(inSoundSect
        ...
        private void Search ()
        {
            ...
            {
                bool searchingTag = !string.IsNullOrEmpty(searchTag);
                bool searchingClip = searchAudioClip != null;
                
                if (searchingTag && !searchingClip)
                    newShowValuesCollection.Add(soundData, soundData.Tag.ToLower().Contains(searchTag.ToLower()));
                else if (!searchingTag && searchingClip)
                    newShowValuesCollection.Add(soundData, soundData.Clips.Any(clip => clip == searchAudioClip));
                else if (searchingTag && searchingClip)
                    newShowValuesCollection.Add(soundData,
                        soundData.Tag.ToLower().Contains(searchTag.ToLower()) &&
